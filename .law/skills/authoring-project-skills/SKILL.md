...
The body below forbids both. CONSTITUTION.md §10 lists "Skipping research" as a named agent failure mode; §3 declares adapters non-authoritative and forbids encoding project law inside them. Both anchors bind every line of this skill.
...
5. **Author at the runtime-discoverable path.** Most runtimes in 2026 load skills at session start from a fixed directory tree; the current research determines the exact path for the active runtime (commonly `.claude/skills/<name>/SKILL.md`, with the kit's canonical copy at `.law/skills/<name>/SKILL.md`). Record the runtime path in the skill's pressure-test envelope.
...
- claim the skill is active in the current session — most runtimes load skills at session start, not dynamically
...
| "The runtime picks up skills automatically — no reload needed." | Most runtimes load skills at session start. Dropping the reload line leaves the user thinking the skill is live when it is not. Surface the line. |
...
Constitution anchors: CONSTITUTION.md §3 (Authority — adapters are non-authoritative projections; skills are adapter-class), §10 (Agent failure model — "Skipping research" and "Overfitting to familiar patterns" both apply to skill authoring).
