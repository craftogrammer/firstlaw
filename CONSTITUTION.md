...
| 2026-04-19 | v1.5.2: narrowed `check-skill-voice` scope to `.law/skills/` only — previous scope included `.claude/skills/` which policed unrelated user-installed Claude Code skills (`playwright-cli`, `project-conventions`, etc.) not written to firstlaw's voice contract. The bridge is a runtime projection; voice compliance stays a kit-source concern. | firstlaw v1.5.2 |
